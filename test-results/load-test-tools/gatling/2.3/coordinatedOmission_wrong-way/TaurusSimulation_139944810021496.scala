...
class TaurusSimulation_139944810021496 extends Simulation {
  ...
  var httpConf = http.baseURL("http://172.31.10.4:3001")
    .header("Accept-Encoding", "gzip")

  var testScenario = scenario("Taurus Scenario")

  var execution = exec(
    http("/api/json/coordinatedOmission").get("/api/json/coordinatedOmission")
      .check(
        status.is(200)
      )
  )
